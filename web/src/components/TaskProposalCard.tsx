...
export function TaskProposalCard({ proposal, busy = false, onApprove, onDrop }: Props) {
  ...
  return <article style={{ border: '1px solid var(--border)', borderRadius: 7, background: 'var(--surface)', padding: 14, display: 'flex', flexDirection: 'column', gap: 9 }}>
    ...
    <div style={{ fontSize: 11, color: 'var(--text2)' }}><b>Evidence candidates</b><div>{proposal.evidence_candidates.length ? proposal.evidence_candidates.map((item, index) => <span key={index} style={{ marginRight: 6 }}>{item.label || item.type || `Candidate ${index + 1}`}</span>) : 'None listed'}</div></div>
    <div style={{ display: 'flex', gap: 7, justifyContent: 'flex-end', paddingTop: 4 }}>
      ...
      <button disabled={busy} onClick={() => { if (editing) { onApprove(proposal, { title, body }); } else setEditing(true); }} className="mono" style={{ padding: '6px 9px', background: 'transparent', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 4 }}>{editing ? 'Edit and approve' : 'Edit and approve'}</button>
      {!editing && <button disabled={busy} onClick={() => onApprove(proposal, {})} className="mono" style={{ padding: '6px 9px', background: 'var(--accent)', color: 'var(--bg)', border: '1px solid var(--accent)', borderRadius: 4 }}>Approve</button>}
    </div>
  </article>;
}
