  ...
  return <div style={page}><div style={{ maxWidth: needsReview ? 1000 : 620, width: '100%' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {selectedSources.length > 0 && <StepCard n={2} active={!installAcknowledged && markerStatus !== 'ok'} done={installAcknowledged || markerStatus === 'ok'} title="Install Builder&apos;s Diary">
        {markerStatus === 'ok' || installAcknowledged ? <div className="mono" style={{ fontSize: 12, color: 'var(--accent)' }}>✓ Installed</div> : <><p style={body}>Run this once. It installs the import skill in Claude Code.</p><div style={commandRow}><code className="mono" style={code}>{installCommand(selectedSources)}</code><button onClick={() => copy(installCommand(selectedSources), 'install')} className="mono" style={secondaryButton}>{copied === 'install' ? '✓ Copied' : 'Copy'}</button></div><button onClick={() => setInstallAcknowledged(true)} className="mono" style={primaryButton(false)}>Installed — continue</button></>}
      </StepCard>}
      ...
      {markerStatus === 'ok' && <StepCard n={4} active done={false} title="Import your portfolio">{needsReview ? <><p style={body}>Your import is ready. Confirm the projects you want in your portfolio.</p><ImportSelectionTable onSaved={onComplete} refreshKey={importRefreshKey} /></> : <>{latestRun && <ImportProgress runs={importRuns} toolId={selectedSources[0] || 'claude'} />}{captureRoute === 'choose' && <RouteChoices onBulk={() => setCaptureRoute('bulk')} onIndividual={() => setCaptureRoute('individual')} />}{captureRoute === 'bulk' && <BulkRoute copied={copied} copy={copy} onBack={() => setCaptureRoute('choose')} />}{captureRoute === 'individual' && <IndividualRoute copied={copied} copy={copy} onBack={() => setCaptureRoute('choose')} />}</>}</StepCard>}
    </div></div></div>;
}
function RouteChoices({ onBulk, onIndividual }: { onBulk: () => void; onIndividual: () => void }) { return <><p style={body}>How would you like to add work?</p><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}><button onClick={onBulk} style={routeButton(false)}><strong>Bulk import</strong><span style={routeDescription}>Bring in past conversations and sessions.</span></button><button onClick={onIndividual} style={routeButton(false)}><strong>Add one conversation</strong><span style={routeDescription}>Put the regular skill in your current client chat and use it there.</span></button></div></>; }
function BulkRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Open a new Claude Code chat and use the import skill. Open the current Claude Code chat and use the import skill if you are already there.</p><button onClick={() => copy(IMPORT_PROMPT, 'import')} className="mono" style={smallPrimary}>{copied === 'import' ? '✓ Copied' : 'Copy /builders-diary-import'}</button></div>; }
function IndividualRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Use the regular skill in your current client chat with <code className="mono">/builders-diary</code>; this does not open a new Claude Code window.</p><button onClick={() => copy(RECORD_PROMPT, 'record')} className="mono" style={smallPrimary}>{copied === 'record' ? '✓ Copied' : 'Copy /builders-diary'}</button></div>; }
...
const code: React.CSSProperties = { display: 'block', flex: 1, minWidth: 0, padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 4, fontSize: 11.5, overflowX: 'auto' };
const commandRow: React.CSSProperties = { display: 'flex', alignItems: 'stretch', gap: 8, marginBottom: 14 };
const secondaryButton: React.CSSProperties = { alignSelf: 'center', padding: '7px 10px', background: 'transparent', color: 'var(--text2)', border: '1px solid var(--border)', borderRadius: 4, cursor: 'pointer', fontSize: 11 };
