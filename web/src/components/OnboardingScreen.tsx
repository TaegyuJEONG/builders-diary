...
function BulkRoute({ copied, copy, folderPath, viewerUrl, onBack }: { copied: string | null; copy: (text: string, key: string) => void; folderPath?: string; viewerUrl?: string; onBack: () => void }) { const prompt = skillPrompt(IMPORT_PROMPT, folderPath, viewerUrl); return <div><BackButton onClick={onBack} /><p style={body}>Open a new Claude Code chat and use the import skill.</p><p style={hint}>Keep this page open and place it next to the Claude Code window. You will confirm proposals here while the import runs in that chat.</p><div style={commandBox}><code className="mono" style={code}>{prompt}</code><button onClick={() => copy(prompt, 'import')} aria-label="Copy import skill" title="Copy import skill" className="mono" style={secondaryButton}>{copied === 'import' ? '✓' : '⧉'}</button></div><ModelRecommendation /></div>; }
