...
function IndividualRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Open the Claude chat, Cowork space, or Claude Code session you want to turn into a portfolio entry, then use the regular skill.</p><div style={commandBox}><code className="mono" style={code}>{RECORD_PROMPT}</code><button onClick={() => copy(RECORD_PROMPT, 'record')} aria-label="Copy regular skill" title="Copy regular skill" className="mono" style={secondaryButton}>{copied === 'record' ? '✓' : '⧉'}</button></div></div>; }
