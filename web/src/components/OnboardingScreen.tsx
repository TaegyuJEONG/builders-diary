...
export function OnboardingScreen({ onSelectFolder, isLoading, error, folderConnected, connectNonce = 0, folderPath, selectedClients, setSelectedClients, onComplete, importRuns = [], importRefreshKey = 0 }: OnboardingScreenProps) {
  ...
  const importMode = Boolean(latestRun);
  ...
  return <div style={page}><div style={{ maxWidth: needsReview ? 1000 : 620, width: '100%' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {!importMode && selectedSources.length > 0 && <StepCard n={2} active={!installAcknowledged && markerStatus !== 'ok'} done={installAcknowledged || markerStatus === 'ok'} title="Install Builder&apos;s Diary">
...
function SideBySideCard() { return <div style={recommendation}><strong>Keep this page open</strong><span>Place it next to the Claude Code window. You will confirm proposals here while the import runs in that chat.</span></div>; }
