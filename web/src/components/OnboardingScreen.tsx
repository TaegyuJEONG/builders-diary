...
function BulkRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Open the current Claude Code chat and use the import skill.</p><button onClick={() => copy(IMPORT_PROMPT, 'import')} className="mono" style={smallPrimary}>{copied === 'import' ? '✓ Copied' : 'Copy /builders-diary-import'}</button></div>; }
