  ...
  return <div style={page}><div style={{ maxWidth: needsReview ? 1000 : 620, width: '100%' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {selectedSources.length > 0 && <StepCard n={2} active={!installAcknowledged && markerStatus !== 'ok'} done={installAcknowledged || markerStatus === 'ok'} title="Install Builder&apos;s Diary">
        {markerStatus === 'ok' || installAcknowledged ? <div className="mono" style={{ fontSize: 12, color: 'var(--accent)' }}>✓ Installed</div> : <><p style={body}>Run this once. It installs the import skill in Claude Code.</p><code className="mono" style={code}>{installCommand(selectedSources)}</code><button onClick={() => setInstallAcknowledged(true)} className="mono" style={primaryButton(false)}>Installed — continue</button></>}
      </StepCard>}
      ...
      {markerStatus === 'ok' && <StepCard n={3} active={false} done title="Connect your folder"><div className="mono" style={{ fontSize: 12, color: 'var(--accent)' }}>✓ Folder connected</div></StepCard>}
...
function RouteChoices({ onBulk, onIndividual }: { onBulk: () => void; onIndividual: () => void }) { return <><p style={body}>How would you like to add work?</p><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}><button onClick={onBulk} style={routeButton(false)}><strong>Bulk import</strong><span>Bring in past conversations and sessions.</span></button><button onClick={onIndividual} style={routeButton(false)}><strong>Add one conversation</strong><span>Capture one conversation as you work.</span></button></div></>; }
function BulkRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Open a new Claude Code chat and use the import skill.</p><p style={hint}>It checks the selected apps’ known local locations only after you start the import; it does not read their content until you confirm.</p><button onClick={() => copy(IMPORT_PROMPT, 'import')} className="mono" style={smallPrimary}>{copied === 'import' ? '✓ Copied' : 'Copy /builders-diary-import'}</button></div>; }
function IndividualRoute({ copied, copy, onBack }: { copied: string | null; copy: (text: string, key: string) => void; onBack: () => void }) { return <div><BackButton onClick={onBack} /><p style={body}>Open a new Claude Code chat, then start your work with <code className="mono">/builders-diary</code>.</p><button onClick={() => copy(RECORD_PROMPT, 'record')} className="mono" style={smallPrimary}>{copied === 'record' ? '✓ Copied' : 'Copy /builders-diary'}</button></div>; }
