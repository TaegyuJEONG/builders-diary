  ...
  return <div style={page}><div style={{ maxWidth: needsReview ? 1000 : 620, width: '100%' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {selectedSources.length > 0 && <StepCard n={2} active={!installAcknowledged && markerStatus !== 'ok'} done={installAcknowledged || markerStatus === 'ok'} title="Install Builder&apos;s Diary">
        {markerStatus === 'ok' || installAcknowledged ? <div className="mono" style={{ fontSize: 12, color: 'var(--accent)' }}>✓ Installed</div> : <><p style={body}>Run this once. It installs the import skill in Claude Code.</p><div style={commandBox}><code className="mono" style={code}>{installCommand(selectedSources)}</code><button onClick={() => copy(installCommand(selectedSources), 'install')} className="mono" style={secondaryButton}>{copied === 'install' ? '✓ Copied' : 'Copy'}</button></div><button onClick={() => setInstallAcknowledged(true)} className="mono" style={primaryButton(false)}>Installed — continue</button></>}
      </StepCard>}
