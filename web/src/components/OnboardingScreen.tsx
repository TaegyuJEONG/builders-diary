...
function skillPrompt(prompt: string, folderPath?: string) {
  const folderName = (folderPath || 'connected-folder-name').replace(/["\\\r\n]/g, '');
  return `${prompt}\nPortfolio folder: "$HOME/Documents/${folderName}"`;
}
function BulkRoute({ copied, copy, folderPath, onBack }: { copied: string | null; copy: (text: string, key: string) => void; folderPath?: string; onBack: () => void }) { const prompt = skillPrompt(IMPORT_PROMPT, folderPath); return <div><BackButton onClick={onBack} /><p style={body}>Open a new Claude Code chat and use the import skill.</p><div style={commandBox}><code className="mono" style={code}>{prompt}</code><button onClick={() => copy(prompt, 'import')} aria-label="Copy import skill" title="Copy import skill" className="mono" style={secondaryButton}>{copied === 'import' ? '✓' : '⧉'}</button></div><ModelRecommendation /></div>; }
function IndividualRoute({ copied, copy, folderPath, onBack }: { copied: string | null; copy: (text: string, key: string) => void; folderPath?: string; onBack: () => void }) { const prompt = skillPrompt(RECORD_PROMPT, folderPath); return <div><BackButton onClick={onBack} /><p style={body}>Open the Claude chat, Cowork space, or Claude Code session you want to turn into a portfolio entry, then use the regular skill.</p><div style={commandBox}><code className="mono" style={code}>{prompt}</code><button onClick={() => copy(prompt, 'record')} aria-label="Copy regular skill" title="Copy regular skill" className="mono" style={secondaryButton}>{copied === 'record' ? '✓' : '⧉'}</button></div><ModelRecommendation /></div>; }
